...
function updateUserProfileV3(){
    ...
    _updateUserProfile($fname, $lname, $country, $password);
}
function updateUserForgetPwd(){
    ...
    $ok = _updateUserProfile($_POST['fname'], $_POST['lname'], $_POST['country'], $_POST['password']);
    if($ok){
...
{
    global $CONN;
    $allProjects = $CONN->fetchAll("SELECT project_name, Pro_Role, added_by, added_time FROM pro_usr_rel LEFT JOIN projects ON pro_usr_rel.Pro_ID = projects.project_id_number
...
function addUser()
{
    ...
    if (!empty($orgDetails['orgtype'])) { //new organization
        $resp = jogetUserRegistration( $jogetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'], $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName'], $orgDetails['orgDescription'] );
        $resp1 = jogetUserRegistration($jogetAssetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'], $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName'], $orgDetails['orgDescription'] );
        if($userDetails['support_user'] == true && $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP){
            $resp2 = jogetUserRegistration($jogetSupportHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'], $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName'], $orgDetails['orgDescription'] );
            $response['support'] = $resp2;
        }
    } else {
        $resp = jogetUserRegistration($jogetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'],  $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName']);
        $resp1 = jogetUserRegistration($jogetAssetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'], $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName']);
        if($userDetails['support_user'] == true && $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP){
            $resp2 = jogetUserRegistration($jogetSupportHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'],  $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName'] );
            $response['support'] = $resp2;
        }
    }
    $myresp = json_decode($resp);
    $myresp1 = json_decode($resp1);
    if ($myresp == "" || $myresp1 == "") { //user was not created in one of the joget. so exit
        $response['bool'] = false;
        $response['msg'] = "Unable to add the user to the system. Please check the connection.";
        $response['data'] = "close";
        return;
    } else {
        if ($myresp->message != "User Created" || $myresp1->message != "User Created" ) { //user was not created in joget. so exit
            $response['bool'] = false;
            ...
            if($userDetails['support_user']  && $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP){
                $myresp2 = json_decode($resp2);
                $response['msg2'] = $myresp2->message ;
               // if ($myresp2->message != "User Created" || $myresp2->message != "User Updated"){
                    // $response['bool'] = false;
                    // $response['msg'] = $myresp->message ;
                    // $response['msg1'] = $myresp1->message ;
                    //$response['msg2'] = $myresp2->message ;
            ...
            $insertUserSQL = "INSERT INTO users (" . implode(" ,", $colsToInsertUser) . ",created_by, created_time) VALUES (" . implode(" ,", $varsToInsertUser) . ", '$email', GETDATE())";
...
{ // to validate if the email already exists or not
    global $CONN;
    $return = array();
    if ($userEmail != null) {
...
function updateUser()
{
   ...
   $resp ="";
    if (!empty($userDetails['user_password'])) {
        $resp = jogetUserUpdate($jogetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail, $userDetails['user_password']);
        $resp1 = jogetUserUpdate($jogetAssetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail,  $userDetails['user_password']);
        if(isset($support_user_change) && $support_user_change == true){ //either added or removed as support user
            ...
            if($userDetails['support_user'] == true){// user is a support user . so update the details
                if( $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP){ //support joget is not the same as construct or asset joget
                    ...
                    $resp2 = jogetUserUpdate($jogetSupportHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail, $userDetails['user_password']);
        ...
        if(isset($support_user_change) && $support_user_change == true){ //either added or removed as support user
            ...
            if($userDetails['support_user'] == true){// user is a support user . so update the details
                if( $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP){ //support joget is not the same as construct or asset joget
                    ...
                    $resp2 = jogetUserUpdate($jogetSupportHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail);
        ...
        $resp = jogetUserUpdate($jogetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail);
        $resp1 = jogetUserUpdate($jogetAssetHostIP, $userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail);
    }
    $myresp = json_decode($resp);
    $myresp1 = json_decode($resp1);
    if ($myresp == "" || $myresp1 == "") { //user was not updated in joget. so exit
        ...
        if ($myresp->message != "User Updated" || $myresp1->message != "User Updated") { //user was not updated in one or both jogets. so exit
            ...
            if ($email == $fetchUserEmail) { // changing logged in users details
                if (!empty($userDetails['user_password'])) {
                    session_destroy();
                    $response['msg'] = "All users were added to the system successfully. Current session is destroyed due to change in password. please relogin.";
                    $response['data'] = "close";
                    $response['joget'] = $resp;
                    return;
                }
...
function inactivateUser()
{
    ...
    $result1 = jogetUserInactivateActivateDelete($jogetAssetHostIP, $userList);
   if( $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP && sizeof($userSupportList) > 0){
...
function deleteUser()
{
    ...
    $myresp = json_decode($result);
    $myresp1 = json_decode($result1);
    if ($myresp == "" || $myresp1 =="") { //user/users were not deleted in joget .. so exit
...
function activeUsers()
{
    ...
    $getSql = $CONN->fetchAll("SELECT user_firstname, user_lastname, user_email, user_org, user_country , user_type FROM users WHERE user_type!= 'non_active' ORDER BY user_id");
...
function updateTheme()
{
    ...
    global $response;
    global $CONN;
    $theme = filter_input(INPUT_POST, 'themeValue', FILTER_VALIDATE_INT);

    $user = $_SESSION['email'];
    $updSql = "UPDATE users SET theme_preference = :0 WHERE user_email='$user'";
    $ok = $CONN->execute($updSql, array($theme));
...
function _updateUserProfile($fname, $lname, $country, $password)
{
    if (!isset($_SESSION['email'])) {
        $response['bool'] = false;
        $response['msg'] = "Insufficient parameter";
    ...
    if (isset($_POST['password'])) {
        $resp = jogetUserUpdate($jogetHostIP, $fname, $lname, $email, $password);
        $resp1 = jogetUserUpdate($jogetAssetHostIP, $fname, $lname, $email, $password);
        if( $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP){
            ...
            if($support_user){
                $resp2 = jogetUserUpdate($jogetSupportHostIP, $fname, $lname, $email, $password);
                $myresp2 = json_decode($resp2);
        ...
        if ($myresp == ""  || $myresp1 == "") { //user was not updated in joget. so exit
            ...
            if (isset($_POST['bentleyusername']) && isset($_POST['bentleypassword'])) {
                ...
                last_update =GETDATE(), bentley_username = :5, bentley_password = :6 WHERE user_email= :7";
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $password, $email, $bentleyusername, $bentleypassword, $email));
            } else {
                $updSql = "UPDATE users SET user_firstname = :0, user_lastname = :1, user_country =:2, user_password = :3,updated_by = :4,
                last_update =GETDATE() WHERE user_email= :5";
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $password, $email, $email));
            }
        ...
        $resp = jogetUserUpdate($jogetHostIP, $fname, $lname, $email);
        $resp1 = jogetUserUpdate($jogetAssetHostIP, $fname, $lname, $email);
        if( $jogetAssetHostIP != $jogetSupportHostIP && $jogetSupportHostIP != $jogetHostIP){
            //check if the user is support user first
            $support_user =  $CONN->fetchOne("SELECT support_user FROM users WHERE user_email =:0", array($email));
            if($support_user){
                $resp2 = jogetUserUpdate($jogetSupportHostIP, $fname, $lname, $email);
                $myresp2 = json_decode($resp2);
                if ($myresp2 == "" ) { //user was not updated in joget. so exit
                    $response['msg'] = "Unable to update to the joget system. please check the joget connection.";
                    $response['data'] = "close";
        ...
        if ($myresp == "" || $myresp1 == "") { //user was not updated in joget. so exit
            ...
            if (isset($_POST['bentleyusername']) && isset($_POST['bentleypassword'])) {
                ...
                $updSql = "UPDATE users SET user_firstname = :0, user_lastname = :1, user_country =:2,updated_by = :3,
                    last_update =GETDATE(), bentley_username = :4, bentley_password = :5 WHERE user_email= :6";
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $email, $bentleyusername, $bentleypassword, $email));
            } else {

                $updSql = "UPDATE users SET user_firstname = :0, user_lastname = :1, user_country =:2,updated_by = :3,
                    last_update =GETDATE() WHERE user_email= :4";
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $email, $email));
            }
...
function getAllUsers()
{
    ...
    if ($project_id !== 'None') {
        ...
        $fetchUserList = $CONN->fetchAll("SELECT u.user_id, u.user_email, u.user_firstname, u.user_lastname, o.orgName, u.user_country, u.user_type, u.support_user, o.orgType,
...
function refreshDelUserTableBody (){
    ...
    $refSql = "SELECT user_id, user_email, user_firstname, user_lastname, user_org, user_country, user_type FROM users WHERE user_type = 'non_active' ORDER By user_id";
