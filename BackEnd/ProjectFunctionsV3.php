...
function viewProject()
{
    ...
    if (!$response['data']['parent_project_id_number']) {
        ...
        $response['app_list'] = null;
         //get the region details from the parent to populate the select options for edit for Asset projects
         if($response['data']['project_type'] == "ASSET"){
            $response['data']['parent_region'] = getParentRegion($response['data']['parent_project_id_number']);
        }
    }
    return $response;
}
...
function getParentRegion($project_id_number){
    global $CONN;
    return $CONN->fetchOne("SELECT region from projects WHERE project_id_number= :0", array($project_id_number));
}
...
{ //deleteproject.php
    ...
    foreach ($fetchedProjectIds as $key => $pid) {
        ...
        if($checkRunningProcess == true){
            ...
            if ($joget_response && $joget_response->message == "Cleanup Completed") {
    ...
    foreach ($users as $user) {
        ...
        if ($functionType === "Create") {
            ...
            if ($user_old_role != "noOldRole") { //then need to remove from database
                $detachUser = unAssignUserToProject($pidnumber, $userId, $userEmail, $user_old_role, $userOrgSubType);
                if (isset($detachUser)) {
                    if ($detachUser == false) {
...
function ProjectAppParams($pidnumber)
{
    ...
    if(filter_input(INPUT_POST, 'contract_level', FILTER_SANITIZE_STRING)){
        $appLinks['app_CP_level'] = filter_input(INPUT_POST, 'contract_level', FILTER_SANITIZE_STRING);
    }
...
function updateProjectDetails(){
    ...
    if(isset($_POST['warrantyenddate'])){
        $projectDetails['warranty_end_date'] = filter_input(INPUT_POST, 'warrantyenddate', FILTER_SANITIZE_STRING);
    }
...
function refreshProjTableBody(){
    ...
    $selectSql = "SELECT project_id_number, project_type, project_id, project_name,industry,location, parent_project_id_number, project_owner,
