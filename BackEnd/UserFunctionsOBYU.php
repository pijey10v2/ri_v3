...
function updateUserProfileV3(){
    ...
    _updateUserProfile($fname, $lname, $country, $phone);
}
...
function updateUserForgetPwd(){
    ...
    $_POST['user_phone'] = $usr['user_phone'];
    $_POST['password'] = $user_pwd;

    $ok = _updateUserProfile($_POST['fname'], $_POST['lname'], $_POST['country'], $_POST['user_phone']);
    if($ok){
...
function addUser()
{
    ...
    if (!empty($orgDetails['orgtype'])) { //new organization
        $resp = jogetUserRegistration($userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'], $jogetSup, $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName'], $orgDetails['orgDescription'], $orgDetails['orgConstructs']);
    } else {
        $resp = jogetUserRegistration($userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'], $jogetSup, $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName']);
        // call jogetk api
        if ($userDetails['support_user'] == true) {
            $support_resp = jogetUserRegistration($userDetails['user_firstname'], $userDetails['user_lastname'], $userDetails['user_email'],  $jogetSup, $userDetails['user_password'], $orgDetails['orgID'], $orgDetails['orgName'], "", "SUPPORT");
        }
    }
        ...
        if ($myresp->message != "User Created") { //user was not created in joget. so exit
            ...
            if (!empty($orgDetails['orgType'])) {
                ...
                if (!$fetchOrg) {
                    ...
                    if (!$ok) {
                        $response['bool'] = false;
                        $response['msg'] = "User added to system but Unable to add the organization due to error in sql :" . $CONN->errorMsg;
                        $response['data'] = "close";
                        return;
                    }
                }
            ...
            $response['msg'] = "User successfully added to the joget system and the database.";
            $response['data'] = "close";
            $response['joget'] = $resp;
        }
    }
...
function updateUser()
{
    ...
    if (!empty($userDetails['user_password'])) {
        $resp = jogetUserUpdate($userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail,  $jogetSup, $userDetails['user_password']);
        ...
        if (isset($support_user_change) && $support_user_change == true) { //either added or removed as support user
            if ($userDetails['support_user'] == true) {// user has been added as support user - add to support joget
                ...
                $support_resp = jogetUserRegistration($userDetails['user_firstname'], $userDetails['user_lastname'], $usr['user_email'],  $jogetSup, $userDetails['user_password'], $usr['orgID'], $usr['orgName'], $usr['orgDescription'], "SUPPORT");
            } else {
                ...
                $support_resp = jogetUserInactivateActivateDelete($userList, "SUPPORT");
            }
            ...
            if ($userDetails['support_user'] == true) {// user is a support user . so update the details
                $support_resp = jogetUserUpdate($userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail,  $jogetSup, $userDetails['user_password'], "", "SUPPORT");
            }
        }
        ...
        if (isset($support_user_change) && $support_user_change == true) { //either added or removed as support user
            ...
            if ($userDetails['support_user'] == true) {// user is a support user . so update the details
                $support_resp = jogetUserUpdate($userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail, $jogetSup, "", "", "SUPPORT");
            }
        }

        $resp = jogetUserUpdate($userDetails['user_firstname'], $userDetails['user_lastname'], $fetchUserEmail,  $jogetSup);
    }
    $myresp = json_decode($resp);
    if ($myresp == "") { //user was not updated in joget. so exit
        $response['msg'] = "Unable to update the user to the system. Please check the connection.";
        $response['data'] = "close";
        return;
    } else {
        if ($myresp->message != "User Updated") { //user was not updated in joget. so exit
            $response['msg'] = $myresp->message . " User not updated";
            $response['data'] = "close";
            return;
        } else { // update the user to DB now
            if (!empty($userDetails['user_password'])) {
                $userDetails['user_password'] = password_hash($userDetails['user_password'], PASSWORD_DEFAULT);
            }
...
function inactivateUser()
{
    ...
    $userList = [];
    $jogetUserUpdateList = [];
    $fetchUserEmails = $CONN->fetchAll("SELECT user_email FROM users WHERE user_id IN ($idsStr) AND user_email !=:0", array($email));
    ...
    $support_result = jogetUserInactivateActivateDelete($userList, "SUPPORT");
    ...
    if ($myresp == "") { //user/users were not de activated .. so exit
        ...
        if (!empty($jogetUserUpdateList)) {
            ...
            $deletesql = "DELETE from pro_usr_rel WHERE Usr_ID IN ($id_nums)";
...
function reactivateUser()
{
    ...
    $userList = [];
    $jogetUserUpdateList = [];
    $fetchUserEmails = $CONN->fetchAll("SELECT user_email FROM users WHERE user_id IN ($idsStr)");
    foreach ($fetchUserEmails as $key => $uemail) {
        array_push($userList, (object) [
            'userName' => $uemail['user_email'],
            'status' => 'active',
...
function updateUserProfile(){
    ...
    _updateUserProfile($fname, $lname, $country, $mobileno);
}


function _updateUserProfile($fname, $lname, $country, $mobileno)
{
    if (!isset($_SESSION['email'])) {
        $response['bool'] = false;
        $response['msg'] = "Insufficient parameter";
    ...
    global $CONN;
    $email = $_SESSION['email'];
    $jogetSup = $_SESSION['support_user'];
    ...
    if (isset($_POST['password'])) {
        ...
        $resp = jogetUserUpdate($fname, $lname, $email,  $jogetSup, $password);
        if (isset($_SESSION['support_user']) && $_SESSION['support_user'] == true ){// user is a support user . so update the details
            $support_resp = jogetUserUpdate($fname, $lname, $email,  $jogetSup, $password, "", "SUPPORT");
        }
        $myresp = json_decode($resp);
        if ($myresp == "") { //user was not updated in joget. so exit
            ...
            if (isset($_POST['bentleyusername']) && isset($_POST['bentleypassword'])) {
                ...
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $password, $email, $bentleyusername, $bentleypassword, $mobileno, $email));
            } else {
                $updSql = "UPDATE users SET user_firstname = :0, user_lastname = :1, user_country =:2, user_password = :3,updated_by = :4,
                last_update =GETDATE(), user_phone =:5 WHERE user_email= :6";
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $password, $email, $mobileno, $email));
            }
        ...
        $resp = jogetUserUpdate($fname, $lname, $email,  $jogetSup);
        if (isset($_SESSION['support_user']) && $_SESSION['support_user'] == true ){// user is a support user . so update the details
            $support_resp = jogetUserUpdate($fname, $lname, $email,  $jogetSup, "", "", "SUPPORT");
        }
        $myresp = json_decode($resp);
        if ($myresp == "") { //user was not updated in joget. so exit
            $response['msg'] = "Unable to update to the joget system. please check the joget connection.";
            ...
            if (isset($_POST['bentleyusername']) && isset($_POST['bentleypassword'])) {
                ...
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $email, $bentleyusername, $bentleypassword, $mobileno, $email));
            } else {

                $updSql = "UPDATE users SET user_firstname = :0, user_lastname = :1, user_country =:2, updated_by = :3,
                    last_update =GETDATE(), user_phone =:4 WHERE user_email= :5";
                $ok = $CONN->execute($updSql, array($fname, $lname, $country, $email, $mobileno, $email));
            }
...
function adminUserTableRefresh (){
    ...
    $pid = $_SESSION['project_id'];
    $ownerOrg = $_SESSION['project_owner'];
...
function refreshDelUserTableBody (){
    ...
    $refSql = "SELECT user_id, user_email, user_firstname, user_lastname, user_org, user_country, user_type, support_user FROM users WHERE user_type = 'non_active' ORDER By user_id";
    $refreshProject = $CONN->fetchAll($refSql);
    $response = $refreshProject;
    return $response;
}

function getOrgOwnerMember(){
    global $response;
    global $CONN;
    ...
    // filter user list by parent id if available
    if ($parentId) {
        $fetchUserList = $CONN->fetchAll("SELECT u.user_id, u.user_email, u.user_firstname, u.user_lastname, o.orgName, u.user_country, u.user_type, o.orgID, o.orgType FROM users u , organization o WHERE user_type != 'non_active' AND u.user_org = o.orgID AND o.orgID IN('OBYU', :0) and exists (select 1 from pro_usr_rel pur where pur.Usr_ID = u.user_id and pur.Pro_ID = :1 ) ORDER By user_id", array($ownerId, $parentId));
    }else{
        $fetchUserList = $CONN->fetchAll("SELECT u.user_id, u.user_email, u.user_firstname, u.user_lastname, o.orgName, u.user_country, u.user_type, o.orgID, o.orgType FROM users u , organization o WHERE user_type != 'non_active' AND u.user_org = o.orgID AND o.orgID IN('OBYU', :0) ORDER By user_id", array($ownerId));
    }
