...
var videoR = new Resumable({
	target: '../JS/uploader/storeDatav3.php',
	testChunks: true
});
...
$(document).ready(function (){ //need this to show the browse file option
	...
	$('#startCamFile').click(function(){
		...
		setInstruction('<div class="instruction rightClick"><i class="fa-solid fa-computer-mouse"></i><label>  Right click to mark text.</label></div>\
                        <div class="instruction escape"><label>Press <kbd>Esc</kbd> to exit.</label></div>', 'drawTool')
...
progressBar2 = new ProgressBar2($('.progress-bar'))
...
function validateVideo(fileName, callback){
	$.ajax({			// to check if the physical file already exist in server 
		type: "POST",
		url: '../JS/uploader/validateVideov3.php',
		dataType: 'json',
		data: {
			fileName : fileName
			},
