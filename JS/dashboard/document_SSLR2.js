function conOpLink(process, status, thirdParty){
    ...
    switch (process) {
        case 'DOC':
            ...
            linkParamArr = processFilterParamArr([status, searchfilter.dateFrom, searchfilter.dateTo, '', '', '', searchfilter.section])
			if(status == 'Active'){
				cardName = 'Total'
			}else{
			cardName = status
		...
		case 'COR':
			...
			if(status == ''){
				...
				if(thirdParty == 'No'){
					cardName = 'Incoming Internal'
				}else{
					cardName = 'Incoming Third Party'
				}
			}else if(status == 'Outgoing'){
				if(thirdParty == 'No'){
					cardName = 'Outgoing Internal'
				}else{
            ...
            events: {
                click: function (event) {
					if(localStorage.ui_pref == 'ri_v3'){
						...
						linkParamArr = processFilterParamArr(['Active', searchfilter.dateFrom, searchfilter.dateTo, event.point.name, '', '', searchfilter.section])
...
function drawSubTypeOfDocumentChart(data, monthYear){
	...
	var chart = Highcharts.chart('typeOfdocument', {
		...
		tooltip: {
		  ...
		  pointFormat: '<span style="color:#4d4d4d">{point.name}</span>: <b>{point.y}</b><br/>'
		},
		...
		events: {
			click: function (event) {
				if(localStorage.ui_pref == 'ri_v3'){
					...
					linkWinTitle = 'Document'
					linkName = 'dash_doc_register_doc_card'
					linkParamArr = processFilterParamArr(['Active', searchfilter.dateFrom, searchfilter.dateTo, 'Drawing', event.point.name, '', searchfilter.section])
					window.parent.widgetConopOpen('', linkName, linkParamArr, linkWinTitle +' - '+ event.point.name);
				}
			}
		}
				...
				events: {
					click: function (event) {
						if(localStorage.ui_pref == 'ri_v3'){
							...
							linkWinTitle = 'Document'
							linkName = 'dash_doc_register_doc_card'
							linkParamArr = processFilterParamArr(['Active', searchfilter.dateFrom, searchfilter.dateTo, 'Drawing', '', '', searchfilter.section])
				...
				events: {
					click: function (event) {
						if(localStorage.ui_pref == 'ri_v3'){
							...
							linkWinTitle = 'Document'
							linkName = 'dash_doc_register_doc_card'
							linkParamArr = processFilterParamArr(['Active', searchfilter.dateFrom, searchfilter.dateTo, 'Drawing', '', 'Yes', searchfilter.section])
...
function updateCoresspondenceCard(ttl = 0, inc = 0, out = 0, incInternal = 0, outInternal = 0) {
	var CorrespondenceTotalCard = ttl;
	var CorrThirdPartyInCard = inc;
	var CorrThirdPartyOutCard = out;
	var CorrThirdPartyTotalCard = "";
	var ThirdPartyTotalCard = parseInt(inc)+parseInt(out);
	var CorrInternalInCard = incInternal;
	var CorrInternalOutCard = outInternal;
	var CorrInternalTotalCard = "";
	var InternalTotalCard = parseInt(incInternal)+parseInt(outInternal);
	...
	if(CorrThirdPartyInCard > 0){
		CorrThirdPartyInCard = `<span class="clickableCard incoming-text" onclick="conOpLink('COR','Incoming', 'Yes', '')">`+inc+`</span>`;
	}

	if(CorrThirdPartyOutCard > 0){
		CorrThirdPartyOutCard = `<span class="clickableCard outgoing-text" onclick="conOpLink('COR','Outgoing', 'Yes', '')">`+out+`</span>`;
	}

	if(ThirdPartyTotalCard > 0){
		CorrThirdPartyTotalCard = `<span class="clickableCard" onclick="conOpLink('COR','', 'Yes', '')">`+ThirdPartyTotalCard+`</span>`;
	}

	if(CorrInternalInCard > 0){
		CorrInternalInCard = `<span class="clickableCard incoming-text" onclick="conOpLink('COR','Incoming', 'No' ,'')">`+incInternal+`</span>`;
	}

	if(CorrInternalOutCard > 0){
		CorrInternalOutCard = `<span class="clickableCard outgoing-text" onclick="conOpLink('COR','Outgoing', 'No' ,'')">`+outInternal+`</span>`;
	}

	if(InternalTotalCard > 0){
		CorrInternalTotalCard = `<span class="clickableCard" onclick="conOpLink('COR','', 'No' ,'')">`+InternalTotalCard+`</span>`;
	}

    $('#CorrespondenceTotalCard').html(CorrespondenceTotalCard);
	$('#CorrThirdPartyInCard').html(CorrThirdPartyInCard);
	$('#CorrThirdPartyOutCard').html(CorrThirdPartyOutCard);
	$('#CorrThirdPartyTotalCard').html(CorrThirdPartyTotalCard);
	$('#CorrInternalInCard').html(CorrInternalInCard);
	$('#CorrInternalOutCard').html(CorrInternalOutCard);
	$('#CorrInternalTotalCard').html(CorrInternalTotalCard);
			...
			events: {
				click: function (event) {
					if(localStorage.ui_pref == 'ri_v3'){
						...
						linkName = 'dash_doc_register_corr_card'
						linkParamArr = processFilterParamArr(['', 'No', searchfilter.dateFrom, searchfilter.dateTo, event.point.name, searchfilter.section])
						window.parent.widgetConopOpen('', linkName, linkParamArr, linkWinTitle +' - '+ event.point.name);
					}
				}
			}
...
function refreshInformation(packId = 'overall', sectionId = 'overall', year = 'all', month = 'all'){
	...
	var InternalCardOut = (corrData.cardNoThirdParty && corrData.cardNoThirdParty[year] && corrData.cardNoThirdParty[year][month] && corrData.cardNoThirdParty[year][month]['Outgoing']) ? corrData.cardNoThirdParty[year][month]['Outgoing'] : 0;
	updateCoresspondenceCard(docCardTtl, docCardInc, docCardOut, InternalCardInc, InternalCardOut);
